...
public:
  ...
  jclass loadClass(JNIEnv *env, const char *class_name) {
    ...
    LOG_JH_DEBUG("Using ClassLoader to look up '%s'", binaryName.c_str());
...
void JavaClassUtils::setJavaClassLoader(JNIEnv *env) {
  LOG_JH_DEBUG("Finding Java ClassLoader");
...
jclass JavaClassUtils::findClass(JNIEnv *env, const char *class_name, bool useClassLoader) {
  jclass result = NULL;
  if (useClassLoader) {
    LOG_JH_DEBUG("Finding class '%s' with thread ClassLoader", class_name);
    if (sJavaClassLoader == NULL) {
      LOG_JH_DEBUG("Java ClassLoader is null, doing lazy initialization");
    ...
    LOG_JH_DEBUG("Finding class '%s' with direct JNI call", class_name);
    result = env->FindClass(class_name);
    JavaExceptionUtils::checkException(env);
  }

  LOG_JH_DEBUG("Lookup of class was %s", result != NULL ? "successful" : "unsuccessful");
  if (result == NULL) {
