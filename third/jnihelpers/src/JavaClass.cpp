...
_fields(NULL),
_clazz_global(NULL) {
  LOG_JH_DEBUG("Creating new empty instance of class");
}
...
_clazz_global(NULL) {
  ...
  LOG_JH_DEBUG("Initializing new instance of class");
  _methods = &_methods_global;
  _fields = &_fields_global;
}

JavaClass::~JavaClass() {
  LOG_JH_DEBUG("Destroying instance of class");
}
...
void JavaClass::merge(const JavaClass *globalInstance) {
  LOG_JH_DEBUG("Merging instance of '%s' with global class info", getSimpleName());
...
void JavaClass::setJavaObject(JNIEnv *env, jobject javaThis) {
  LOG_JH_DEBUG("Setting fields from Java object of type '%s' to native instance", getSimpleName());
  ...
  if (_field_mappings.empty()) {
   ...
   for (FieldMap::const_iterator iter = _fields->begin(); iter != _fields->end(); ++iter) {
    ...
    LOG_JH_DEBUG("Copying field '%s'", key.c_str());
    ...
    if (field != NULL && mapping != NULL) {
      if (TYPE_EQUALS(mapping->type, kTypeInt)) {
        ...
        *address = env->GetCharField(javaThis, field);
        ...
        LOG_JH_ERROR("Unable to copy data to field '%s'", key.c_str());
      }
    }
  }
}
...
jobject JavaClass::toJavaObject(JNIEnv *env) {
  LOG_JH_DEBUG("Converting native instance of '%s' to Java instance", getSimpleName());
...
void JavaClass::setClass(JNIEnv *env) {
  LOG_JH_INFO("Looking up corresponding Java class for '%s'", getCanonicalName());
...
void JavaClass::cacheConstructor(JNIEnv *env) {
  LOG_JH_DEBUG("Caching constructor in class '%s'", getSimpleName());
  if (!isInitialized()) {
    JavaExceptionUtils::throwExceptionOfType(env, kTypeIllegalStateException,
...
void JavaClass::cacheMethod(JNIEnv *env, const char* method_name, const char* return_type, ...) {
  LOG_JH_DEBUG("Caching method '%s' in class '%s'", method_name, getSimpleName());
  if (!isInitialized()) {
    JavaExceptionUtils::throwExceptionOfType(env, kTypeIllegalStateException,
      "Attempt to call cacheMethod without having set class info");
    return;
...
void JavaClass::cacheField(JNIEnv *env, const char *field_name, const char *field_type) {
  LOG_JH_DEBUG("Caching field '%s' (type %s) in class '%s'", field_name, field_type, getSimpleName());
  if (!isInitialized()) {
    JavaExceptionUtils::throwExceptionOfType(env, kTypeIllegalStateException,
...
void JavaClass::addNativeMethod(const char *method_name, void *function, const char *return_type, ...) {
  LOG_JH_DEBUG("Adding native method '%s' to class '%s'", method_name, getSimpleName());
  ...
  LOG_JH_DEBUG("Native signature is '%s'", nativeMethod.signature);
  va_end(arguments);
...
bool JavaClass::registerNativeMethods(JNIEnv *env) {
  LOG_JH_DEBUG("Registering native methods on class '%s'", getSimpleName());
