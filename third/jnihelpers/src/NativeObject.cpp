...
bool NativeObject::persist(JNIEnv *env, jobject javaThis) {
  if (isInitialized()) {
    LOG_JH_DEBUG("Persisting instance of '%s' to Java object", getSimpleName());
    if (javaThis == NULL) {
    ...
    LOG_JH_DEBUG("Persist was successful");
    return true;
  }
  return false;
}
...
JavaClass* NativeObject::getPersistedInstance(JNIEnv *env, jobject javaThis) const {
  if (isInitialized()) {
    LOG_JH_DEBUG("Retrieving persisted instance of '%s'", getSimpleName());
...
void NativeObject::destroy(JNIEnv *env, jobject javaThis) {
  if (isInitialized()) {
    LOG_JH_DEBUG("Destroying persisted instance of '%s'", getSimpleName());
    if (javaThis == NULL) {
      JavaExceptionUtils::throwExceptionOfType(env, kTypeIllegalArgumentException,
