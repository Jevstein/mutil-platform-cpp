...
ClassRegistry::ClassRegistry() {
  LOG_JH_DEBUG("Creating new class registry");
}

ClassRegistry::~ClassRegistry() {
  LOG_JH_DEBUG("Destroying class registry");
}
...
void ClassRegistry::add(JNIEnv *env, const JavaClass *item) {
  if (item == NULL) {
    ...
    LOG_JH_INFO("Adding class instance '%s' to registry", item->getCanonicalName());
    _classes[item->getCanonicalName()].set(item);
  }
}

const JavaClass* ClassRegistry::get(const char* name) const {
  LOG_JH_DEBUG("Looking up class named '%s'", name);
  if (name == NULL) {
